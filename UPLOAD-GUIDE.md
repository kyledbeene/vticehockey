...
Edit `games.csv`. Do not edit the score cards in `scores.html`; those cards are fallback markup only.
...
The report page gets the matchup and score from `games.csv`, and gets the title, summary, date, and recap from the Markdown file. Update the score in `games.csv` separately; do not put the score in the report file as a replacement for the CSV score.
...
- Scores are updated in `games.csv`, not in static HTML.
